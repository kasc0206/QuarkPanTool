...
- 🚀 多线程分片下载：基于 HTTP Range 的多线程分片下载，速度提升 5-12 倍。
- 📁 网盘文件直接下载：支持从用户自己的网盘直接下载已转存的文件（`download_from_pan.py`）。
- ☁️ iCloud 自动转移：下载完成后自动将文件复制到 iCloud（`move_to_icloud.py`）。
...
#### 🚀 多线程分片下载（v0.0.8+）
使用 HTTP Range 分片并发下载，将单文件切割为多个 50MB 的块同时下载：
- 小文件（<50MB）自动使用单线程
- 大文件自动启用多线程并发（最多 16 线程）
- 速度从单线程 ~60KB/s 提升至 ~700KB/s

#### 📁 直接下载网盘文件
```bash
python download_from_pan.py
```
自动列出网盘根目录文件，选择要下载的文件夹即可。可跳过已存在于 iCloud 中的文件。

#### ☁️ iCloud 自动转移
```bash
python move_to_icloud.py
```
等待下载完成后，自动将 PDF 文件复制到 iCloud 指定目录。
